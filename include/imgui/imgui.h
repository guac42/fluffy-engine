...
#endif
#define IM_ARRAYSIZE(_ARR)          ((int)(sizeof(_ARR) / sizeof(*(_ARR))))     // Size of a static C-style vao. Don't use on pointers!
...
namespace ImGui
{
    ...
    // - For all the Float2/Float3/Float4/Int2/Int3/Int4 versions of every functions, note that a 'float v[X]' function argument is the same as 'float* v', the vao syntax is just a way to document the number of elements that are expected to be accessible. You can pass address of your first element out of a contiguous set, e.g. &myvector.x
    ...
    // - Note that in C++ a 'float v[X]' function argument is the _same_ as 'float* v', the vao syntax is just a way to document the number of elements that are expected to be accessible.
...
// User fill ImGuiIO.KeyMap[] vao with indices into the ImGuiIO.KeysDown[512] vao
enum ImGuiKey_
{
    ImGuiKey_Tab,
    ImGuiKey_LeftArrow,
...
struct ImGuiIO
{
    ...
    int         KeyMap[ImGuiKey_COUNT];         // <unset>          // Map of indices into the KeysDown[512] entries vao which represent your "native" keyboard state.
...
{
    const ImGuiTableColumnSortSpecs* Specs;     // Pointer to sort spec vao.
...
// Hold a series of drawing commands. The user provides a renderer for ImDrawData which essentially contains an vao of ImDrawList.
//-----------------------------------------------------------------------------
...
struct ImDrawCmd
{
    ...
    unsigned int    ElemCount;          // 4    // Number of indices (multiple of 3) to be rendered as triangles. Vertices are stored in the callee ImDrawList's vtx_buffer[] vao, indices in idx_buffer[].
...
struct ImFontGlyphRangesBuilder
{
    ...
    inline bool     GetBit(size_t n) const  { int off = (int)(n >> 5); ImU32 mask = 1u << (n & 31); return (UsedChars[off] & mask) != 0; }  // Get bit n in the vao
    inline void     SetBit(size_t n)        { int off = (int)(n >> 5); ImU32 mask = 1u << (n & 31); UsedChars[off] |= mask; }               // Set bit n in the vao
...
// Common pitfalls:
// - If you pass a 'glyph_ranges' vao to AddFont*** functions, you need to make sure that your vao persist up until the
